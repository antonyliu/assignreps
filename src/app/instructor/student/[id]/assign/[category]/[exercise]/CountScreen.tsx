...
type Props = {
  ...
  playerName: string;
  categorySlug: string;
  /** The category's display title, for the back link. Resolved server-side
   *  from CATEGORIES alongside the exercise, the same way exerciseName is —
   *  rather than re-looked-up here, so one place owns that mapping. */
    ...
    return (
      <main className="flex flex-col min-h-screen items-center justify-center text-center px-6">
        <div
          ...
          style={{ background: "rgba(62,214,138,0.12)" }}
        >
          <Check size={34} color="#3ed68a" strokeWidth={2.5} />
        </div>
        <p
    ...
    <main className="flex flex-col min-h-screen p-[1.75rem_1.25rem]">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        >
          ...
          <span className="text-[14px] font-medium text-reps-ink">{categoryTitle}</span>
        </Link>
        <Link
      ...
      <h2 className="text-2xl font-semibold tracking-[-0.5px]">{exerciseName}</h2>
      <p className="text-[12px] text-reps-sub mt-0.5 mb-8">Assigning to {playerName}</p>
