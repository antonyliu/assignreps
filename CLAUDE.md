...
- **Bullets:** Assign homework to a student / They get a text, log it there / You know exactly what got done
  ...
  - ⚠️ **`.bullet-text` is `white-space: nowrap`, so bullet copy never wraps — it widens the layout instead.** `.landing-text` is `flex: 1` with the default `min-width: auto`, so the longest bullet sets a min-content floor for the whole text column. Slack on the longest of the three: **27px at 375**, **0px at 768**, comfortable from 820. Treat ~30 characters at 20px as the practical ceiling for any future bullet.
  - ⚠️ **Known: a narrow horizontal-overflow band around 768px, and it is PRE-EXISTING.** The old copy overflowed 5px there too; this copy is ~20px wider at 20px type, so it deepens that to 25px and widens the band from roughly 768–772 to roughly 768–793. Every other tested width — 375, 390, 414, 1024, 1280, 1440 — is clean, as is 820+. **The fix, when wanted, is to drop `white-space: nowrap`**: the bullets would then wrap only inside the pinched band and render identically everywhere else. Not done as part of the copy change because the hero is locked against visual changes.
