...
**`/privacy`:**
- ⚠️ **Stripe is not named as a processor.** The vendor list is Supabase, Twilio, Resend. Stripe has handled payment data since Aug 1–4 and is a real subprocessor. This is a factual gap in a disclosure section.
- ⚠️ **"What we collect" is out of date.** It omits **student notes** (`logs.note` — free text written by a minor, shipped Aug 1) and **billing data** (`stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, added Aug 1).
...
- **Player-delete cascade is unmentioned.** Removing a player permanently destroys every assignment and log that student wrote (`players.delete()`, and `logs.player_id` stays `ON DELETE CASCADE`). The in-app modal is honest about it; no public page is. Glossed **by omission**, not by a false statement.
...
- **Says "instructors"**, where public copy standardised on "coaches & trainers" on Aug 5. The internal `instructor` vocabulary is deliberate; Terms is public.
