...
8. **Remaining privacy-audit bundle.** ✅ **Phase 1 (accuracy) done Aug 17 2026** — Stripe named, student notes and billing fields disclosed, the player-delete cascade and deactivation covered, the account-deletion promise made honest about being manual, `/terms` re-worded to "coaches and trainers", and `/faq`'s advertiser over-claim trimmed. ✅ **Phase 2 (minors) done Aug 17 2026.** ⚠️ **Still open, in order:** contrast — `/terms` still carries the failing `#378add` throughout and `#888888` fails on the "Last updated" line of **both** pages — then the date bump, once. ⏸️ Three gaps needing NEW terms (refunds, user termination, stating the plan limits) are deferred as a group; see *Legal pages* below.
9. ⚠️ **Coaches never see or accept `/terms` — nothing in signup links to it.** Verified Aug 17 2026: the entire signup tree contains no reference to `/terms` or `/privacy` — no link, no checkbox, no "by continuing you agree". Both pages are reachable only from the public landing footer, which a coach arriving by direct link may never see.
   - ⚠️ **This undercuts every "you are responsible" clause on `/terms`**, not just one. The SMS verbal-consent requirement is the load-bearing example: Twilio's toll-free registration rests on it, and it is currently an obligation asserted against someone who has never been shown the document asserting it.
   - **Not a copy fix.** The wording is fine; what is missing is the moment of agreement. Smallest honest version is a line on the final signup step — "By continuing you agree to our Terms and Privacy Policy", both linked — which is also where *First-touch trust at signup* wants a trust line, so the two are worth doing together.
   - ⚠️ Found while rewriting `/privacy`'s minors section, which had been resting on that non-existent agreement. That section no longer does; the rest of `/terms` still does.
10. **Landing page metadata** (title/description/OG — **five strings**, four in `page.tsx` plus the fallback in `layout.tsx`). Parked, needs real thought: SEO wants "practice/drills" keywords, but "practice" collides with basketball's team-session meaning.
11. ⚠️ **`saveAssignment()` and `saveCustomAssignment()` do not verify the player belongs to the coach.** Both insert with `coach_id: user.id` and a **client-supplied `player_id`**, leaning on RLS alone rather than reading the row first — unlike `repeatAssignment()`, which is player-scoped, and `addPlayer()`, which counts server-side. **Found Aug 17 2026 while building deactivation and deliberately left**, to keep that build scoped; the pause check added to both actions goes through `requireActivePlayer()`, which *is* ownership-scoped, so it happens to close the gap for those two calls — but the insert itself is still unguarded and would remain so if that check were ever removed. Its own small pass: read the player scoped by `coach_id` and refuse if absent, in both actions.
12. ⚠️ **CLAUDE.md date error — the Aug 6 / Aug 16 mixup.** `/faq`, the privacy fixes and the testimonial removal are labelled **"Aug 6" throughout this file**, but git dates every one of those commits **Aug 16** (`c675f39`, `f5ccbff`, `7416c69`, `b5f4db0`, `ab6a7f3`). The cause is visible in the log: there is a clean ten-day gap with no commits between `72469a8` (Aug 5) and `c675f39` (Aug 16), matching the *"⏸️ Where things stand — Aug 6, pausing for a while"* note — so the pause date got carried forward as the session's own date when work resumed. Roughly a dozen references need correcting, across the `/faq` section, the legal-pages audit and the Landing page section. Found Aug 17 2026 while backfilling `CHANGELOG.md`, which uses the git dates and is therefore correct where the two disagree. **Not urgent — but the case study should use the right dates**, so this wants its own pass rather than being fixed opportunistically.
13. **CLAUDE.md restructure** — split current state from narrative history into `docs/`. Its own isolated pass.
14. **Roster view at scale — backburner design question.** Testing the 30-student Pro ceiling surfaced that a flat card list of 30 feels overwhelming to scan, even though most of a coach's actual attention only lives in two of the four status groups (Done / In progress). ⚠️ **Some of that reaction was likely the test data itself** — 24 identically-named placeholder rows with no real variation — rather than the list format; real names and real activity patterns would read differently. But the underlying question is worth keeping: whether a card list is the right format for scanning thirty players at once, versus something more spreadsheet-like. **Revisit once a coach actually approaches that number organically, not from seeded test data.**
15. **Later, no urgency:** the signed-in app is capped at phone width on desktop/tablet; landing page modules feel rigid block-to-block (TeuxDeux-style overlap was the direction); an animated hero walkthrough.
...
**Both:**
- ⚠️ **Still dated "Last updated: July 17, 2026"** — deliberately NOT bumped by the Aug 6 fix, because more privacy edits are pending (Stripe, minors, collection scope) and bumping twice in a week is noise. **Bump it once when the rest lands.** ⚠️ Do not let the page ship to strangers with substantive Aug 6 edits under a July date.
- ⚠️ **AA: 9 failures on each page**, measured. Two failing colours, not one: `#378add` at **3.36:1** (all `h2`s at 17px/600, the back link and the mailto at 15px) and **`#888888` at 3.31:1** on the "Last updated" line — that second one was missed by the earlier finding, which named only the brand blue. `/faq` solved the same problem with `#2a6fb5` (4.86:1) and `#1a1a1a`.
- Privacy's intro says *"built by one person"*; `/faq` moved to **"Let us know"** on Aug 6. The two now read as different entities.
