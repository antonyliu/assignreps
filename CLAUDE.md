...
- **Section 2's copy undersells what RJ actually values.** It currently sells organisation and visibility: *"Your whole program, finally in one place."* That is real but generic.
  ...
  - ⚠️ **This is a PRODUCT decision to reopen, not a copy tweak, and that is the whole reason it is parked rather than queued.** *Product decisions locked* states: *"Default exercise libraries are the product experience. Custom creation is the escape hatch."* Writing the copy RJ's language points at would promote custom-drill creation from escape hatch to **headline-level selling point** — a direct inversion of that hierarchy. The copy cannot be written honestly until the hierarchy question is answered, because the page would be selling something the product deliberately treats as secondary.
  - **The question to actually answer, before a word is drafted:** *does the default-library-first hierarchy still hold?* It was locked when the library WAS the product and no one had used it in anger. RJ has since built a real practice on it and describes it in ownership terms. Either the lock still stands and section 2 keeps selling organisation, or it does not and the product's own emphasis shifts with the copy. Both are defensible; picking one is the work.
  - ⚠️ **If the answer is "custom leads", copy is not the first thing to change.** Custom exercise creation is currently only reachable **inside the assign flow for a specific player** — RJ has separately asked for it to be reachable on its own, and that is unbuilt (see Pending). Headlining a coach's own drill library while the only way to build one is mid-assign would be selling a smoother product than exists. Sequence is: decide the hierarchy → close that gap if needed → then write.
  - **Not started. Blocked on the decision above, not on writing ability.**
